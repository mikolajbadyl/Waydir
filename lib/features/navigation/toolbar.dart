...
class Toolbar extends StatelessWidget {
  ...
  const Toolbar({super.key, required this.store, required this.notificationStore, this.shellStore});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _ToolBtn(PhosphorIconsRegular.arrowUp, store.goUp, true, t.toolbar.up),
          Container(width: 1, height: 16, color: AppColors.bgDivider, margin: const EdgeInsets.symmetric(horizontal: 4)),
          _ToolBtn(PhosphorIconsRegular.arrowClockwise, store.refresh, true, t.toolbar.refresh),
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _hovered && widget.enabled ? AppColors.bgHover : Colors.transparent,
...
class _PathBarState extends State<_PathBar> {
  ...
  Widget _buildBreadcrumbs(String path) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      for (int i = 0; i < segments.length; i++) {
        final w = caretWidth + segW(segments[i], last: i == segments.length - 1);
        widths.add(w);
        total += w;
      }
      ...
      if (total <= maxW) {
        return Row(
          ...
          children: [
            if (hasRoot)
              _BreadcrumbSegment(label: '/', onTap: () => widget.store.navigateTo('/')),
            for (int i = 0; i < segments.length; i++) ..._segmentRow(segments, i, isLast: i == segments.length - 1, flexibleLast: false),
          ],
        );
      }
      ...
      return Row(
        children: [
          ...
          for (int k = 0; k < shown.length; k++) ...() {
            ...
            return _segmentRow(segments, i, isLast: isLast, flexibleLast: isLast);
          }(),
        ],
      );
    });
  ...
  List<Widget> _segmentRow(List<String> segments, int i, {required bool isLast, required bool flexibleLast}) {
    final partial = '${Platform.pathSeparator}${segments.take(i + 1).join(Platform.pathSeparator)}';
    ...
    return [
      _caretIcon(),
      flexibleLast ? Flexible(child: segment) : segment,
    ];
  }
  ...
  Widget _caretIcon() {
    return Padding(
      ...
      child: PhosphorIcon(PhosphorIconsRegular.caretRight, size: 14, color: AppColors.fgSubtle),
    );
  }
}
...
class _EllipsisMenuState extends State<_EllipsisMenu> {
  ...
  void _openMenu() {
    ...
    final offset = box.localToGlobal(Offset(0, box.size.height + 2), ancestor: overlay);
    ...
    showContextMenu(
      ...
      onSelect: (action) {
        ...
        final partial = '${Platform.pathSeparator}${widget.allSegments.take(i + 1).join(Platform.pathSeparator)}';
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: _hovered
              ? BoxDecoration(color: AppColors.bgHover, borderRadius: BorderRadius.circular(3))
              : null,
          child: Tooltip(
            message: widget.hiddenIndices.map((i) => widget.allSegments[i]).join(' › '),
            child: Text(
              '…',
...
class _BreadcrumbSegmentState extends State<_BreadcrumbSegment> {
  ...
  Widget build(BuildContext context) {
    ...
    final text = Text(
      ...
      maxLines: 1,
      softWrap: false,
      overflow: widget.ellipsisOverflow ? TextOverflow.ellipsis : TextOverflow.clip,
      style: context.txt.body.copyWith(
        color: widget.isLast
            ? AppColors.fg
...
class _ViewOptionsButtonState extends State<_ViewOptionsButton> {
  ...
  void _openMenu() {
    final box = context.findRenderObject() as RenderBox;
    final overlay = Overlay.of(context).context.findRenderObject() as RenderBox;
    final offset = box.localToGlobal(Offset(0, box.size.height), ancestor: overlay);
